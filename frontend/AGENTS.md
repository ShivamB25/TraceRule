...
│   ├── api.ts             # Fetch-based API client (6 functions, base=/api/v1)
...
| `uploadPolicy` | async fn | `api.ts` | `POST /api/v1/policies/upload` (FormData) |
| `getRules` | async fn | `api.ts` | `GET /api/v1/rules` with optional status/policy_id |
| `approveRule` | async fn | `api.ts` | `PATCH /api/v1/rules/{id}/approve` |
| `rejectRule` | async fn | `api.ts` | `PATCH /api/v1/rules/{id}/reject` |
| `getViolations` | async fn | `api.ts` | `GET /api/v1/violations` with optional rule_id/status |
| `triggerScan` | async fn | `api.ts` | `POST /api/v1/scan` |
...
- **Polling**: Manual `setInterval` in useEffect. Compilation polls at 3s (40 attempts max). Explanation polls at 5s until all non-null.
...
- **Refs**: `useRef` for StrictMode dedup (`initialLoadLoggedRef`, `explanationsPendingRef`) and DOM refs.
...
- **No tests** in frontend. Backend has pytest suite.
...
- **API base**: Hardcoded `'/api/v1'` in `api.ts`. Vite proxy handles routing to backend in dev. In production, configure reverse proxy.
...
- **Polling stops** automatically: compilation polling after rules appear (or 40 attempts), explanation polling when all `ai_explanation` fields are non-null.
